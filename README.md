...
The appsettings snippet above defines a flag named `MyFlag` that is enabled for telemetry. This is indicated by the `Telemetry` object which sets `Enabled` to true. The value of the `Enabled` property must be `true` to publish telemetry for the flag.
...
| `Metadata` | A collection of key-value pairs, modeled as a dictionary, that can be used to add custom metadata on evaluation events that are generated by the flag. |
